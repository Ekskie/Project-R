...
func _input(_event: InputEvent) -> void:
	if Input.is_action_just_pressed("chat"):
		# FIX: If the chat is already open and focused, let the LineEdit type the letter instead of resetting!
	...
	if Input.is_action_just_pressed("pause"):
		if chat_edit.has_focus(): 
			chat_edit.release_focus()
			# Backup: force exit chat if the player presses pause/esc while typing
			_chat_exited()
		else: #pause menu
